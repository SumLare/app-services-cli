...
func (c *KubernetesCluster) Connect(ctx context.Context, cmdOptions *ConnectArguments, opts Options) error {
	...
	case "":
		...
		if cfg.Services.Kafka == nil || cmdOptions.IgnoreContext {
			// nolint
			selectedKafka, err := kafkaUtil.InteractiveSelect(opts.Connection, opts.Logger)
			if err != nil {
				return err
			}
		...
		if cfg.Services.ServiceRegistry == nil || cmdOptions.IgnoreContext {
			// nolint
			selectedServiceRegistry, err := serviceregistry.InteractiveSelect(opts.Connection, opts.Logger)
			if err != nil {
				return err
			}
