...
func (k *Kafka) CreateResource(ctx context.Context, c *KubernetesCluster, namespace string, serviceID string, opts Options) error {
	...
	// return watchForCustomResourceStatus(c, serviceName, namespace, "kafka", opts), serviceName

	// opts.Logger.Info(opts.Localizer.MustLocalize("cluster.kubernetes.watchForKafkaStatus.log.info.wait"))
